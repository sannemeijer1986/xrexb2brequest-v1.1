...
window.StaticAuthGate = (function () {
  const DEFAULTS = {
    passwordSha256Hex: "REPLACE_WITH_YOUR_SHA256_HEX",
  ...
  return {
    /** Run at top of protected pages – redirects to login if not authenticated */
    protect() {
    ...
    /** Generate SHA-256 hex of password (for setup: open console, run StaticAuthGate.hash("your-pw")) */
    async hash(password) {
      return sha256Hex(password);
    },

    /** Check if current request has valid auth cookie */
    isAuthenticated() {
      const c = cfg();
      return hasCookie(c.cookieName, c.cookieValue);
    },

    /** Validate password against configured hash */
    async validatePassword(password) {
      const hash = await sha256Hex(password);
      return hash === cfg().passwordSha256Hex;
    },

    /** Set auth cookie and redirect to next URL (used by login form) */
